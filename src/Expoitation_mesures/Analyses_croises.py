...
def fusionnerDfSources(dfEmissionTheorique, dfMeteo, df6MinBruit, colonnesMeteo=['grad_moy', 'ConditionPropagation', 'forcePropagation_num', 
                                                                  'dir_vent_haut', 'vts_vent_haut', 'rayonnement', 'temp_haut']):
    """
    creer les df pourchaque thematique et les concatener
    colonnesMeteo : liste des colonnes de la dfMeteo a conserver
    """ 
    ...
    df6MinBruit = (df6MinBruit.assign(indicateur='bruit',
                                      jour_semaine=df6MinBruit.date_heure.dt.dayofweek,
    ...
    # partie météo
    dfMeteoChart = dfMeteo.melt(id_vars='date_heure', value_vars=colonnesMeteo, var_name='indicateur', value_name='valeur')
